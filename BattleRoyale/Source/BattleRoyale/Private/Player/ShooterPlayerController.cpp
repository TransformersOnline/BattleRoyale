...
void AShooterPlayerController::ClientGameStarted_Implementation()
{
	...
	if(LocalPlayer != nullptr && Events.IsValid())
	{
		...
		if (UniqueId.IsValid())
		{
			...
			UBattleRoyaleInstance* SGI = GetWorld() != NULL ? Cast<UBattleRoyaleInstance>(GetWorld()->GetGameInstance()) : NULL;
...
{		
	...
	if ( GetNetMode() == NM_Client )
	{
		...
		SGI->ShowMessageThenGotoState( ReturnReason, OKButton, FText::GetEmpty(), BattleRoyaleInstanceState::MainMenu );
	}
	else
	{
		SGI->GotoState(BattleRoyaleInstanceState::MainMenu);
	}
	...
	{	
		...
		if (UniqueId.IsValid())
		{
			...
			if (SGI->GetIsOnline())
			{
				...
				ABattleRoyaleState* const MyGameState = GetWorld() != NULL ? GetWorld()->GetGameState<ABattleRoyaleState>() : NULL;
...
{
	GetWorld()->GetAuthGameMode<ABattleRoyaleMode>()->Broadcast(this, Msg, ServerSayString);
}
...
void AShooterPlayerController::PreClientTravel(const FString& PendingURL, ETravelType TravelType, bool bIsSeamlessTravel)
{
	...
	if ( GetWorld() != NULL )
	{
		UBattleRoyaleViewportClient* ShooterViewport = Cast<UBattleRoyaleViewportClient>( GetWorld()->GetGameViewport() );
