...
ABattleRoyaleSession::ABattleRoyaleSession(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		OnCreateSessionCompleteDelegate = FOnCreateSessionCompleteDelegate::CreateUObject(this, &ABattleRoyaleSession::OnCreateSessionComplete);
		OnDestroySessionCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(this, &ABattleRoyaleSession::OnDestroySessionComplete);

		OnFindSessionsCompleteDelegate = FOnFindSessionsCompleteDelegate::CreateUObject(this, &ABattleRoyaleSession::OnFindSessionsComplete);
		OnJoinSessionCompleteDelegate = FOnJoinSessionCompleteDelegate::CreateUObject(this, &ABattleRoyaleSession::OnJoinSessionComplete);

		OnStartSessionCompleteDelegate = FOnStartSessionCompleteDelegate::CreateUObject(this, &ABattleRoyaleSession::OnStartOnlineGameComplete);
	}
}
 ...
 */
void ABattleRoyaleSession::OnStartOnlineGameComplete(FName InSessionName, bool bWasSuccessful)
{
...
void ABattleRoyaleSession::HandleMatchHasStarted()
{
 ...
 *
 */
void ABattleRoyaleSession::HandleMatchHasEnded()
{
	// start online game locally and wait for completion
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
...
bool ABattleRoyaleSession::IsBusy() const
{ 
...
EOnlineAsyncTaskState::Type ABattleRoyaleSession::GetSearchResultStatus(int32& SearchResultIdx, int32& NumSearchResults)
{
	SearchResultIdx = 0;
	NumSearchResults = 0;
 ...
 */
const TArray<FOnlineSessionSearchResult> & ABattleRoyaleSession::GetSearchResults() const
{ 
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void ABattleRoyaleSession::OnCreateSessionComplete(FName InSessionName, bool bWasSuccessful)
{
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void ABattleRoyaleSession::OnDestroySessionComplete(FName InSessionName, bool bWasSuccessful)
{
...
bool ABattleRoyaleSession::HostSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, const FString& GameType, const FString& MapName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers)
{
...
void ABattleRoyaleSession::OnFindSessionsComplete(bool bWasSuccessful)
{
...
void ABattleRoyaleSession::ResetBestSessionVars()
{
	CurrentSessionParams.BestSessionIdx = -1;
}

void ABattleRoyaleSession::ChooseBestSession()
{
...
void ABattleRoyaleSession::StartMatchmaking()
{
	ResetBestSessionVars();
	ContinueMatchmaking();
}

void ABattleRoyaleSession::ContinueMatchmaking()
{	
	ChooseBestSession();
...
void ABattleRoyaleSession::OnNoMatchesAvailable()
{
	UE_LOG(LogOnlineGame, Verbose, TEXT("Matchmaking complete, no sessions available."));
	SearchSettings = NULL;
}

void ABattleRoyaleSession::FindSessions(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, bool bIsLAN, bool bIsPresence)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
...
bool ABattleRoyaleSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, int32 SessionIndexInSearchResults)
{
	bool bResult = false;
...
bool ABattleRoyaleSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, const FOnlineSessionSearchResult& SearchResult)
{
	bool bResult = false;
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void ABattleRoyaleSession::OnJoinSessionComplete(FName InSessionName, EOnJoinSessionCompleteResult::Type Result)
{
...
bool ABattleRoyaleSession::TravelToSession(int32 ControllerId, FName InSessionName)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
...
void ABattleRoyaleSession::RegisterServer()
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
