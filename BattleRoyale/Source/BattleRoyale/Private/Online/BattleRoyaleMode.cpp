...
#include "Online/BattleRoyaleMode.h"
...
ABattleRoyaleMode::ABattleRoyaleMode(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	GameStateClass = ABattleRoyaleState::StaticClass();
...
FString ABattleRoyaleMode::GetBotsCountOptionName()
{
	return FString(TEXT("Bots"));
}

void ABattleRoyaleMode::InitGame(const FString& MapName, const FString& Options, FString& ErrorMessage)
{
	...
	if (GameInstance && Cast<UBattleRoyaleInstance>(GameInstance)->GetIsOnline())
	{
		bPauseable = false;
	}
}
...
TSubclassOf<AGameSession> ABattleRoyaleMode::GetGameSessionClass() const
{
	return ABattleRoyaleSession::StaticClass();
}

void ABattleRoyaleMode::PreInitializeComponents()
{
	Super::PreInitializeComponents();

	GetWorldTimerManager().SetTimer(TimerHandle_DefaultTimer, this, &ABattleRoyaleMode::DefaultTimer, GetWorldSettings()->GetEffectiveTimeDilation(), true);
}

void ABattleRoyaleMode::DefaultTimer()
{
...
void ABattleRoyaleMode::HandleMatchIsWaitingToStart()
{
...
void ABattleRoyaleMode::HandleMatchHasStarted()
{
	bNeedsBotCreation = true;
...
void ABattleRoyaleMode::FinishMatch()
{
	ABattleRoyaleState* const MyGameState = Cast<ABattleRoyaleState>(GameState);
...
void ABattleRoyaleMode::RequestFinishAndExitToMainMenu()
{
	FinishMatch();

	UBattleRoyaleInstance* const GameInstance = Cast<UBattleRoyaleInstance>(GetGameInstance());
	if (GameInstance)
	{
...
void ABattleRoyaleMode::DetermineMatchWinner()
{
	// nothing to do here
}

bool ABattleRoyaleMode::IsWinner(class AShooterPlayerState* PlayerState) const
{
	return false;
}

void ABattleRoyaleMode::PreLogin(const FString& Options, const FString& Address, const FUniqueNetIdRepl& UniqueId, FString& ErrorMessage)
{
	ABattleRoyaleState* const MyGameState = Cast<ABattleRoyaleState>(GameState);
...
void ABattleRoyaleMode::PostLogin(APlayerController* NewPlayer)
{
...
void ABattleRoyaleMode::Killed(AController* Killer, AController* KilledPlayer, APawn* KilledPawn, const UDamageType* DamageType)
{
...
float ABattleRoyaleMode::ModifyDamage(float Damage, AActor* DamagedActor, struct FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser) const
{
...
bool ABattleRoyaleMode::CanDealDamage(class AShooterPlayerState* DamageInstigator, class AShooterPlayerState* DamagedPlayer) const
{
	return true;
}

bool ABattleRoyaleMode::AllowCheats(APlayerController* P)
{
	return true;
}

bool ABattleRoyaleMode::ShouldSpawnAtStartSpot(AController* Player)
{
	return false;
}

UClass* ABattleRoyaleMode::GetDefaultPawnClassForController_Implementation(AController* InController)
{
...
AActor* ABattleRoyaleMode::ChoosePlayerStart_Implementation(AController* Player)
{
...
bool ABattleRoyaleMode::IsSpawnpointAllowed(APlayerStart* SpawnPoint, AController* Player) const
{
...
bool ABattleRoyaleMode::IsSpawnpointPreferred(APlayerStart* SpawnPoint, AController* Player) const
{
...
void ABattleRoyaleMode::CreateBotControllers()
{
...
AShooterAIController* ABattleRoyaleMode::CreateBot(int32 BotNum)
{
...
void ABattleRoyaleMode::StartBots()
{
...
void ABattleRoyaleMode::InitBot(AShooterAIController* AIController, int32 BotNum)
{	
	if (AIController)
	{
...
void ABattleRoyaleMode::RestartGame()
{
