...
#include "Online/BattleRoyale_TeamDeathMatch.h"
#include "Online/ShooterPlayerState.h"
#include "Bots/ShooterAIController.h"

ABattleRoyale_TeamDeathMatch::ABattleRoyale_TeamDeathMatch(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	NumTeams = 2;
	bDelayedStart = true;
}

void ABattleRoyale_TeamDeathMatch::PostLogin(APlayerController* NewPlayer)
{
...
void ABattleRoyale_TeamDeathMatch::InitGameState()
{
	Super::InitGameState();
...
bool ABattleRoyale_TeamDeathMatch::CanDealDamage(AShooterPlayerState* DamageInstigator, class AShooterPlayerState* DamagedPlayer) const
{
...
int32 ABattleRoyale_TeamDeathMatch::ChooseTeam(AShooterPlayerState* ForPlayerState) const
{
...
void ABattleRoyale_TeamDeathMatch::DetermineMatchWinner()
{
	ABattleRoyaleState const* const MyGameState = Cast<ABattleRoyaleState>(GameState);
...
bool ABattleRoyale_TeamDeathMatch::IsWinner(AShooterPlayerState* PlayerState) const
{
	return PlayerState && !PlayerState->IsQuitter() && PlayerState->GetTeamNum() == WinnerTeam;
}

bool ABattleRoyale_TeamDeathMatch::IsSpawnpointAllowed(APlayerStart* SpawnPoint, AController* Player) const
{
	if (Player)
	{
...
void ABattleRoyale_TeamDeathMatch::InitBot(AShooterAIController* AIC, int32 BotNum)
{	
