		...
		//if (World)
		{
			...
			if (NetDriver)
			{
				switch (FailureType)
				{
					...
					{
						UBattleRoyaleInstance* const ShooterInstance = Cast<UBattleRoyaleInstance>(GameInstance);
						if (ShooterInstance && NetDriver->GetNetMode() == NM_Client)
						{
							...
							// this is the case
							ShooterInstance->ShowMessageThenGotoState( FText::FromString(ErrorString), OKButton, FText::GetEmpty(), BattleRoyaleInstanceState::MainMenu, false );
						}
						break;
					}
					case ENetworkFailure::PendingConnectionFailure:						
					{
						UBattleRoyaleInstance* const GI = Cast<UBattleRoyaleInstance>(GameInstance);
						if (GI && NetDriver->GetNetMode() == NM_Client)
						{
							...
							// We're going to make the assumption that someone else has a better message than "Lost connection to host" if
							// this is the case
							GI->ShowMessageThenGotoState( FText::FromString(ErrorString), OKButton, FText::GetEmpty(), BattleRoyaleInstanceState::MainMenu, false );
						}
						break;
					}
					...
					case ENetworkFailure::ConnectionTimeout:
					{
						...
						if (GI && NetDriver->GetNetMode() == NM_Client)
						{
							...
							// We're going to make the assumption that someone else has a better message than "Lost connection to host" if
							// this is the case
							GI->ShowMessageThenGotoState( ReturnReason, OKButton, FText::GetEmpty(), BattleRoyaleInstanceState::MainMenu, false );
						}
						break;
					}
