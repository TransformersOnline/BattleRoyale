 ...
 */
ABattleRoyaleSession* SShooterServerList::GetGameSession() const
{
	UBattleRoyaleInstance* const GI = Cast<UBattleRoyaleInstance>(PlayerOwner->GetGameInstance());
...
void SShooterServerList::UpdateSearchStatus()
{
	...
	ABattleRoyaleSession* ShooterSession = GetGameSession();
	if (ShooterSession)
	{
			...
			case EOnlineAsyncTaskState::Done:
				// copy the results
				{
					...
					if (NumSearchResults == 0)
					{
						...
						StatusText = LOCTEXT("NoServersFound", "NO SERVERS FOUND, PRESS <img src=\"BattleRoyale.Switch.Left\"/> TO TRY AGAIN");
#else
...
#elif PLATFORM_SWITCH
						StatusText = LOCTEXT("ServersRefresh", "PRESS <img src=\"BattleRoyale.Switch.Left\"/> TO REFRESH SERVER LIST");
#else
