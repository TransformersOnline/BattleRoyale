	...
	[
		...
		[
			...
			[
				...
				.TextStyle(FShooterStyle::Get(), "BattleRoyale.DemoListCheckboxTextStyle")
			]
		]
		+SVerticalBox::Slot()
		.AutoHeight()
...
void SShooterDemoList::DeleteDemo()
{
	...
	if (SelectedItem.IsValid())
	{
		...
		if ( GI != NULL )
		{
			UBattleRoyaleViewportClient* ShooterViewport = Cast<UBattleRoyaleViewportClient>( GI->GetGameViewportClient() );
