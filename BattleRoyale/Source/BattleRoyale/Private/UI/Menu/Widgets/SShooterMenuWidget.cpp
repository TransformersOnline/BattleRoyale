...
void SShooterMenuWidget::Construct(const FArguments& InArgs)
{
	...
	const FSlateFontInfo PlayerNameFontInfo = FShooterStyle::Get().GetWidgetStyle<FTextBlockStyle>("BattleRoyale.MenuProfileNameStyle").Font;
	const FSlateFontInfo ProfileSwapFontInfo = FShooterStyle::Get().GetWidgetStyle<FTextBlockStyle>("BattleRoyale.MenuServerListTextStyle").Font;
	...
	[
		...
		[
			...
			[
				...
				[
					SNew(STextBlock)
					.TextStyle(FShooterStyle::Get(), "BattleRoyale.MenuProfileNameStyle")
				...
				[
					SNew(STextBlock)
					.TextStyle(FShooterStyle::Get(), "BattleRoyale.MenuServerListTextStyle")
					.ColorAndOpacity(MenuTitleTextColor)
					.Text(ProfileSwap)
				]
			]
...
bool SShooterMenuWidget::ProfileUISwap(const int ControllerIndex) const
{
	if(IsProfileSwapActive())
	{
		...
		{
			UBattleRoyaleInstance* GameInstance = PlayerOwner.IsValid() ? Cast< UBattleRoyaleInstance >( PlayerOwner->GetGameInstance() ) : nullptr;
