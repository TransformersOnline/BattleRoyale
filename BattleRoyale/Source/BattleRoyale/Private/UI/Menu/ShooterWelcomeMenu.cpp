	...
	void Construct( const FArguments& InArgs )
	{
		...
		[
			...
			[ 
				...
				.Text(LOCTEXT("PressStartSwitch", "PRESS <img src=\"BattleRoyale.Switch.Right\"/> TO PLAY"))
#else
				.Text( LOCTEXT("PressStartXboxOne", "PRESS A TO PLAY" ) )
#endif
//				.ColorAndOpacity(this, &SShooterWelcomeMenuWidget::GetTextColor)
				.TextStyle( FShooterStyle::Get(), "BattleRoyale.WelcomeScreen.WelcomeTextStyle" )
...
void FShooterWelcomeMenu::Construct( TWeakObjectPtr< UBattleRoyaleInstance > InGameInstance )
{
...
void FShooterWelcomeMenu::HandleLoginUIClosed(TSharedPtr<const FUniqueNetId> UniqueId, const int ControllerIndex)
{
	...
	{
		const FText StopReason	= NSLOCTEXT( "ProfileMessages", "NeedLicense", "The signed in users do not have a license for this game. Please purchase BattleRoyale from the Xbox Marketplace or sign in a user with a valid license." );
...
void FShooterWelcomeMenu::SetControllerAndAdvanceToMainMenu(const int ControllerIndex)
{
	...
	if ( NewPlayerOwner != nullptr && ControllerIndex != -1 )
	{
		...
		GameInstance->GotoState(BattleRoyaleInstanceState::MainMenu);
	}	
}
