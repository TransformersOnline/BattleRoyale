...
void FShooterRecentlyMet::Construct(ULocalPlayer* _PlayerOwner, int32 LocalUserNum_)
{
	...
	UserSettings = CastChecked<UBattleRoyaleUserSettings>(GEngine->GetGameUserSettings());	
}
...
void FShooterRecentlyMet::UpdateRecentlyMet(int32 NewOwnerIndex)
{
	...
	ABattleRoyaleState* const MyGameState = PlayerOwner->GetWorld()->GetGameState<ABattleRoyaleState>();
