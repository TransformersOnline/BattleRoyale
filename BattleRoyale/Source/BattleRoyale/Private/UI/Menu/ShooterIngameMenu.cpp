...
void FShooterIngameMenu::Construct(ULocalPlayer* _PlayerOwner)
{
	...
	const UBattleRoyaleInstance* GameInstance = nullptr;
	if (PlayerOwner)
	{
		GameInstance = Cast<UBattleRoyaleInstance>(PlayerOwner->GetGameInstance());
	}
...
{
	UBattleRoyaleInstance* const GameInstance = Cast<UBattleRoyaleInstance>(PlayerOwner->GetGameInstance());
	if (GameInstance)
	{
