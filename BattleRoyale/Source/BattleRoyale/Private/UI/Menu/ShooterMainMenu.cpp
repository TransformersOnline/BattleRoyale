...
#define LOCTEXT_NAMESPACE "BattleRoyale.HUD.Menu"
...
void FShooterMainMenu::Construct(TWeakObjectPtr<UBattleRoyaleInstance> _GameInstance, TWeakObjectPtr<ULocalPlayer> _PlayerOwner)
{
...
#else
	UBattleRoyaleUserSettings* const UserSettings = CastChecked<UBattleRoyaleUserSettings>(GEngine->GetGameUserSettings());
...
{	
	...
	{
		GameInstance->GotoState(BattleRoyaleInstanceState::WelcomeScreen);
	}
#else
...
{
	UBattleRoyaleViewportClient* ShooterViewport = Cast<UBattleRoyaleViewportClient>(GameInstance->GetGameViewportClient());
	if (ShooterViewport)
	{
...
void FShooterMainMenu::LanMatchChanged(TSharedPtr<FShooterMenuItem> MenuItem, int32 MultiOptionIndex)
{
	...
	UBattleRoyaleUserSettings* UserSettings = CastChecked<UBattleRoyaleUserSettings>(GEngine->GetGameUserSettings());
...
{	
	...
	{
		FString const StartURL = FString::Printf(TEXT("/Game/Maps/%s?game=%s%s%s?%s=%d%s"), *GetMapName(), *GameType, GameInstance->GetIsOnline() ? TEXT("?listen") : TEXT(""), bIsLanMatch ? TEXT("?bIsLanMatch") : TEXT(""), *ABattleRoyaleMode::GetBotsCountOptionName(), BotsCountOpt, bIsRecordingDemo ? TEXT("?DemoRec") : TEXT("") );
...
FReply FShooterMainMenu::OnConfirm()
{
	if (GEngine && GEngine->GameViewport)
	{
		UBattleRoyaleViewportClient * ShooterViewport = Cast<UBattleRoyaleViewportClient>(GEngine->GameViewport);
...
{
	IBattleRoyaleLoadingScreenModule* LoadingScreenModule = FModuleManager::LoadModulePtr<IBattleRoyaleLoadingScreenModule>("BattleRoyaleLoadingScreen");
