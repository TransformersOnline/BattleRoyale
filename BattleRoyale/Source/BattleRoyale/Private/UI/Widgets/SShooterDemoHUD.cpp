	...
	[
		...
		[		
			...
			[
				...
				[
					...
					.BackgroundBrush(FShooterStyle::Get().GetBrush("BattleRoyale.ReplayTimelineBorder"))
					.BackgroundPadding(FMargin(0.0f, 3.0))
					.IndicatorBrush(FShooterStyle::Get().GetBrush("BattleRoyale.ReplayTimelineIndicator"))
				]
		...
		[
			...
			[
				...
				[
					SNew(SImage)
					.Image(FShooterStyle::Get().GetBrush("BattleRoyale.ReplayPauseIcon"))
				]
			]
...
FText SShooterDemoHUD::GetPlaybackSpeed() const
{
	...
	if (PlayerOwner->GetWorldSettings()->Pauser == nullptr)
	{
		...
		return FText::Format(NSLOCTEXT("BattleRoyale.HUD.Menu", "PlaybackSpeed", "{0} X"), FText::AsNumber(PlayerOwner->GetWorldSettings()->DemoPlayTimeDilation, &FormatOptions));
	}

	return NSLOCTEXT("BattleRoyale.HUD.Menu", "Paused", "PAUSED");
}
