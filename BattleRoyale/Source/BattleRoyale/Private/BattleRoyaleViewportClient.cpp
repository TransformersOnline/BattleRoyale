...
UBattleRoyaleViewportClient::UBattleRoyaleViewportClient(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SetSuppressTransitionMessage(true);
}

void UBattleRoyaleViewportClient::NotifyPlayerAdded(int32 PlayerIndex, ULocalPlayer* AddedPlayer)
{
...
void UBattleRoyaleViewportClient::AddViewportWidgetContent( TSharedRef<class SWidget> ViewportContent, const int32 ZOrder )
{
	UE_LOG( LogPlayerManagement, Log, TEXT( "UBattleRoyaleViewportClient::AddViewportWidgetContent: %p" ), &ViewportContent.Get() );
...
void UBattleRoyaleViewportClient::RemoveViewportWidgetContent( TSharedRef<class SWidget> ViewportContent )
{
	UE_LOG( LogPlayerManagement, Log, TEXT( "UBattleRoyaleViewportClient::RemoveViewportWidgetContent: %p" ), &ViewportContent.Get()  );
...
void UBattleRoyaleViewportClient::HideExistingWidgets()
{
...
void UBattleRoyaleViewportClient::ShowExistingWidgets()
{
...
void UBattleRoyaleViewportClient::ShowDialog(TWeakObjectPtr<ULocalPlayer> PlayerOwner, EShooterDialogType::Type DialogType, const FText& Message, const FText& Confirm, const FText& Cancel, const FOnClicked& OnConfirm, const FOnClicked& OnCancel)
{
	UE_LOG( LogPlayerManagement, Log, TEXT( "UBattleRoyaleViewportClient::ShowDialog..." ) );
...
void UBattleRoyaleViewportClient::HideDialog()
{
	UE_LOG( LogPlayerManagement, Log, TEXT( "UBattleRoyaleViewportClient::HideDialog. DialogWidget: %p, OldFocusWidget: %p" ), DialogWidget.Get(), OldFocusWidget.Get() );
...
void UBattleRoyaleViewportClient::ShowLoadingScreen()
{
	if ( LoadingScreenWidget.IsValid() )
	{
		return;
...
void UBattleRoyaleViewportClient::HideLoadingScreen()
{
	if ( !LoadingScreenWidget.IsValid() )
	{
		return;
...
EShooterDialogType::Type UBattleRoyaleViewportClient::GetDialogType() const
{
	return (DialogWidget.IsValid() ? DialogWidget->DialogType : EShooterDialogType::None);
}

TWeakObjectPtr<ULocalPlayer> UBattleRoyaleViewportClient::GetDialogOwner() const
{
	return (DialogWidget.IsValid() ? DialogWidget->PlayerOwner : nullptr);
}

void UBattleRoyaleViewportClient::Tick(float DeltaSeconds)
{
...
#if WITH_EDITOR
void UBattleRoyaleViewportClient::DrawTransition(UCanvas* Canvas)
{
...
void SShooterLoadingScreen::Construct(const FArguments& InArgs)
{
	...
	LoadingScreenBrush = MakeShareable( new FBattleRoyaleLoadingScreenBrush( LoadingScreenName, FVector2D(1920,1080) ) );
